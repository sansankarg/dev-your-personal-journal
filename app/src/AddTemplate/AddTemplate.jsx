...
const AddTemplate = ({ fileData, onFileNameChange }) => {
  ...
  const [editableToggle, setEditableToggle] = useState(true);
  const {user} = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/add-template", {
        ...
        templateLogo: fileData.templateLogo,
        fileName,
  ...
  return (
    <div className="template-container">
      ...
      <div className="header">
        <div className='title'>
          <h3>Set new template</h3>
        </div>
        <div className='title'>
          <h5>Template name : {fileData.templateName}</h5>
        </div>
        <p className="save-status ms-2" onClick={handleSubmit}>
